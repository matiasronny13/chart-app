...
export class PaneRenderer extends PaneRendererBase {
	...
	_drawAlertLabel(scope: BitmapCoordinatesRenderingScope) {
		...
		try {
			...
			const editButtonStartX =
				labelXDimensions.position +
				labelXDimensions.length -
				(removeButtonWidth * 2) * scope.horizontalPixelRatio;

			if (activeLabel.hoverEdit) {
				...
				ctx.fillStyle = '#2994ff';
				ctx.fill();
			}

			const removeButtonStartX = editButtonStartX + removeButtonWidth
			...
			// draw edit button icon
			ctx.beginPath();
			const iconSize = 9;
			ctx.translate(
				editButtonStartX + (scope.horizontalPixelRatio * (removeButtonWidth - iconSize)) / 2,
			...
			ctx.fillStyle = '#131722';
			ctx.fill(editPath, 'evenodd');
			
			// draw remove button icon
			ctx.beginPath();
			ctx.translate(removeButtonWidth, 0);
			ctx.fillStyle = '#131722';
